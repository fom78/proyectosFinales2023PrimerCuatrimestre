...
def sistema_para_usuarios(usuarios, rubros, articulos, stock):
    while True:
        ...
        if opcion=='1':
            abm_articulos(articulos, rubros, stock)  #gestion de articulos
        elif opcion=='2':
            abm_rubros(rubros, articulos)  #gestion de rubros
        elif opcion=='3':
            reportes(articulos, rubros, stock)  #reportes
        elif opcion=='4':
            modo_video('alerta')
...
def menu_principal():
    ...
    menu+=[['com_^', '']]
    menu+=[['com_^', '']]
    menu+=[['com_^', 'Bienvenidos a almacen Manolito!!!', '']]
    menu+=[['com_^', '']]
    menu+=[['com_^', '']]
    menu+=[['com_*']]
    menu+=[['com_n']]
    ...
    menu+=[['com_n']]
    menu+=[['com_n']]
    menu+=[['com_i', 'Presione ENTER para continuar...', '']]
    mostrar_menu(menu)
    while True:
        menu =[['com_*']]
        menu+=[['com_^', 'Sistema de Gestión de Stock', '']]
        ...
        menu+=[['com_n']]
        menu+=[['com_n']]
        menu+=[['com_+', 'Para consultar stock y precio de un artículo ingrese el nombre del mismo']]
        menu+=[['com_n']]
        menu+=[['com_n']]
        menu+=[['com_n']]
        menu+=[['1', 'Listado de rubros']]
        menu+=[['2', 'Ingresar al sistema como un usuario registrado']]
        menu+=[['com_n']]
        menu+=[['com_n']]
        menu+=[['com_n']]
        menu+=[['com_i', 'Ingrese su consulta y luego presione ENTER: ', '']]
        opcion=mostrar_menu(menu)
        if opcion.lower()=='2':  #ingresar al sistema
            ...
            if salir:
                break
        elif opcion.lower()=='1':  #listar rubros
